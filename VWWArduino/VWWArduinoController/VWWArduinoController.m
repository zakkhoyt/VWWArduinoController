...
static NSString *VWWArdruinoErrorDomain = @"VWWArduinoController";
...
-(BOOL)disconnectFromCurrentSerialPort{
    ...
    if (self.serialFileDescriptor != -1) {
        ret = close(self.serialFileDescriptor);
        self.serialFileDescriptor = -1;
    }
...
-(void)writeBytes:(uint8_t*)val length:(NSUInteger)length{
    ...
    if(self.serialFileDescriptor!=-1) {
		write(self.serialFileDescriptor, val, length);
	} else {
...
//                text = [NSString stringWithCString:byte_buffer encoding:NSUTF8StringEncoding];
                text = [NSString stringWithFormat:@"%s", byte_buffer];
                [self receivedString:text];
//                [mText appendFormat:@"%@", text];
